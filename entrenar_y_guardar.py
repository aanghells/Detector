...
# --- CARGAR DATOS DESDE texto.json ---
with open("texto.json", "r", encoding="utf-8") as f:
    datos = json.load(f)

textos = [limpiar_texto(d["texto"]) for d in datos]
emociones = [d["emocion"] for d in datos]

# --- CODIFICAR EMOCIONES ---
le = LabelEncoder()
labels = le.fit_transform(emociones)
...
model.save_pretrained("modelo_emociones_bert")
tokenizer.save_pretrained("modelo_emociones_bert")
with open("label_encoder.pkl", "wb") as f:
    pickle.dump(le, f)

print("✅ Modelo BERT entrenado y guardado con éxito.")
print("Emociones entrenadas:", le.classes_)
